...
class ChannelLocking(commands.Cog):
    ...
    async def lock(self, interaction: discord.Interaction, channel: discord.TextChannel = None):
        ...
        locked = await self.bot.db_pool.fetchval("SELECT locked FROM channels WHERE channel_id = $1", channel.id)
        ...
        if locked:
            embed, file = error_embed(
                ...
                i18n.t("errors.already_locked", locale=language, channel=channel.mention),
        ...
        for key, value in permissions.items():
            overwrites[key] = discord.PermissionOverwrite(view_channel=value.view_channel, send_messages=False if key.is_default() else None)
        ...
        async with self.bot.db_pool.acquire() as conn:
            async with conn.transaction():
                await self.bot.db_pool.execute("INSERT INTO channels (channel_id, synced, locked) VALUES ($1, $2, $3) ON CONFLICT(channel_id) DO UPDATE SET synced = $2, locked = $3", channel.id, channel.permissions_synced, True)
        ...
        embed = discord.Embed(title=i18n.t("moderation.locked_title", channel=channel.name, locale=language), description=i18n.t("moderation.locked_desc", channel=channel.mention, locale=language), color=discord.Color.red())
    ...
    async def unlock(self, interaction: discord.Interaction, channel: discord.TextChannel = None):
        ...
        embed = discord.Embed(title=i18n.t("moderation.unlocked_title", channel=channel.name, locale=language), description=i18n.t("moderation.unlocked_desc", channel=channel.mention, locale=language), color=discord.Color.green())
        if channel_synced:
            category = channel.category
            if category is not None:
                embed.set_footer(text=i18n.t("moderation.unlocked_footer", locale=language))
                await channel.edit(sync_permissions=True, category=category)
                await self.bot.db_pool.execute("UPDATE channels SET locked = $1 WHERE channel_id = $2", False, channel.id)
                return await interaction.response.send_message(embed=embed)
