...
class Roles(commands.GroupCog, name="role"):
    ...
    async def remove_all(self, interaction: discord.Interaction, role: discord.Role, reason: str = None, option: int = 1):
        ...
        if option == 1: # everyone
            embed = discord.Embed(colour=loading_color, description=f"<a:loading:1071777312128905267> Removing the role {role.mention} from {len(member_list)} members...\n**Time left**: soon:tm:") 
            await interaction.response.send_message(embed=embed)
            for user in member_list:
            ...
            embed = discord.Embed(colour=loading_color, description=f"<a:loading:1071777312128905267> Removing the role {role.mention} from {len([m for m in member_list if not m.bot])} humans...\n**Time left**: soon:tm:") 
            await interaction.response.send_message(embed=embed)
            for user in member_list:
                if not user.bot:
                    counter += 1
            ...
            embed = discord.Embed(colour=loading_color, description=f"<a:loading:1071777312128905267> Removing the role {role.mention} from {len([m for m in member_list if m.bot])} humans...\n**Time left**: soon:tm:") 
            await interaction.response.send_message(embed=embed)
            for user in member_list:
                if user.bot:
                    counter += 1
