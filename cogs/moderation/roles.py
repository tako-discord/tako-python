...
class Roles(commands.GroupCog, name="role"):
    ...
    async def add_all(self, interaction: discord.Interaction, role: discord.Role, option: int = 1, reason: str = None):
        ...
        embed = discord.Embed(colour=discord.Colour(loading_color), description=f"<a:loading:1071777312128905267> **Adding** the role {role.mention} to {len(member_list)} members...\nETA: soon:tm:")
        await interaction.response.send_message(embed=embed, ephemeral=True)
        if option == 1: #everyone
            ...
            if counter == 0: embed = discord.Embed(colour=discord.Colour(error_color), description=f"All members are already having the role {role.mention}.")
            ...
            for user in member_list:
                if not user.self.bot:
                    ...
                    if not role in user.roles:
                        counter += 1
                        await user.add_roles(role, reason=f"Role added by {interaction.user}, reason: {reason}")
            if counter == 0: embed = discord.Embed(colour=discord.Colour(error_color), description=f"All humans are already having the role {role.mention}.")
            else: embed = discord.Embed(colour=discord.Colour(success_color), description=f"The role {role.mention} was successfully **given** to {counter} humans!")
            return await interaction.edit_original_response(embed=embed)
            ...
            for user in member_list:      
                if user.bot:
                    ...
                    if not role in user.roles:    
                        counter += 1
                        await user.add_roles(role, reason=f"Role added by {interaction.user}, reason: {reason}")
            if counter == 0: embed = discord.Embed(colour=discord.Colour(error_color), description=f"All bots are already having the role {role.mention}.")
            else: embed = discord.Embed(colour=discord.Colour(success_color), description=f"The role {role.mention} was successfully **given** to {counter} bots!")
            return await interaction.edit_original_response(embed=embed)
    ...
    async def remove_all(self, interaction: discord.Interaction, role: discord.Role, reason: str = None, option: int = 1):
        ...
        embed = discord.Embed(colour=discord.Colour(loading_color), description=f"<a:loading:1071777312128905267> **Removing** the role {role.mention} from {len(member_list)} members...\nETA: soon:tm:")
        await interaction.response.send_message(embed=embed, ephemeral=True)
        if option == 1: # everyone
            for user in member_list:
            ...
            if counter == 0: embed = discord.Embed(colour=discord.Colour(error_color), description=f"No one has the role {role.mention}.")
            else: embed = discord.Embed(colour=discord.Colour(success_color), description=f"The role {role.mention} was successfully **removed** from {counter} members!")
            return await interaction.edit_original_response(embed=embed)
            ...
            if counter == 0: embed = discord.Embed(colour=discord.Colour(error_color), description=f"No one has the role {role.mention}.")        
            else: embed = discord.Embed(colour=discord.Colour(success_color), description=f"The role {role.mention} was successfully **removed** from {counter} humans!")
            return await interaction.edit_original_response(embed=embed)   
            ...
            if counter == 0: embed = discord.Embed(colour=discord.Colour(error_color), description=f"No one has the role {role.mention}.")        
            else: embed = discord.Embed(colour=discord.Colour(success_color), description=f"The role {role.mention} was successfully **removed** from {counter} bots!")
            return await interaction.edit_original_response(embed=embed)
