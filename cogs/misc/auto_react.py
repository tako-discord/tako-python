...
class AutoReact(commands.GroupCog, group_name="auto_react"):
    ...
    async def add(self, interaction: discord.Interaction, emoji: app_commands.Transform[discord.Emoji | str, EmojiTransformer], channel: discord.TextChannel = None): # type: ignore
        ...
        data = await self.bot.db_pool.fetchval("SELECT auto_react FROM channels WHERE channel_id = $1", channel.id)
        if not data:
            data = []
        if str(emoji) not in data:
            data.append(str(emoji))
        await self.bot.db_pool.execute("INSERT INTO channels (channel_id, auto_react) VALUES ($1, $2) ON CONFLICT(channel_id) DO UPDATE SET auto_react = $2", channel.id, data)
        return await interaction.response.send_message(f"Added {emoji} to {channel.mention}", ephemeral=True)
    ...
    async def remove(self, interaction: discord.Interaction, emoji: app_commands.Transform[discord.Emoji | str, EmojiTransformer], channel: discord.TextChannel = None): # type: ignore
        ...
        await self.bot.db_pool.execute("INSERT INTO channels (channel_id, auto_react) VALUES ($1, $2) ON CONFLICT(channel_id) DO UPDATE SET auto_react = $2", channel.id, data)
        return await interaction.response.send_message(f"Removed {emoji} from {channel.mention}", ephemeral=True)
    ...
    async def on_message(self, message: discord.Message):
        data = await self.bot.db_pool.fetchval("SELECT auto_react FROM channels WHERE channel_id = $1", message.channel.id)
        if data:
