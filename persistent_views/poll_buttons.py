...
class PollButtons(discord.ui.View):
    ...
    class StopButton(discord.ui.Button):
        def __init__(
            self, uuid: UUID, question: str, answers: list[str], bot, user_id: int
        ):
            super().__init__(
                label="Stop",
                custom_id=str(bot.user.id) + "poll_stop",
        ...
        async def callback(self, interaction: discord.Interaction):
            ...
            view = PollButtons(
                self.uuid, self.question, self.answers, self.bot, self.user_id
            )
