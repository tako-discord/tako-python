...
class PollButtons(discord.ui.View):
        ...
        class PollButton(discord.ui.Button):
            def __init__(self, uuid: UUID, question: str, answer: str, answers: list[str], bot):
            ...
            async def callback(self, interaction: discord.Interaction):
                async with self.bot.db_pool.acquire() as conn:
                    async with conn.transaction():
                        data = (await conn.fetch("SELECT * FROM polls WHERE id = $1", self.uuid))[0]
                        ...
                        await conn.execute("UPDATE polls SET votes = $1 WHERE id = $2", votes, self.uuid)

                embed = await poll_embed(self.question, self.answers, votes, self.bot, interaction.guild_id)
